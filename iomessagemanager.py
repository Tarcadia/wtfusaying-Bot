...
logger_formatter = logging.Formatter(fmt='\033[0m%(asctime)s \033[1;34m[%(levelname)s]\033[0;32m[%(name)s]\033[0m >> %(message)s', datefmt='%H:%M');
...
def send(mm, data: dict or str = None):
    ...
    with mm['print_lock']:
        if type(data) == str:
            ...
            for _k in data:
                print("\033[0m%5s \033[1;34m[OUT]\033[0m >> \033[0;32m%8s\033[0m : %s" % (_t, _k, data[_k]));
    return mm, True;
